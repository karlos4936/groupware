			...
			<tr>
				...
				<c:choose>
					<c:when test="${draft.isSign1 eq null }">
						<td class="title"><a href="wait/view.do?no=${draft.no}" >${draft.title }</a></td>
					</c:when>
					<c:when test="${draft.isSign1 ne null and draft.isSign3 eq null }">
						<td class="title"><a href="proceed/view.do?no=${draft.no}" >${draft.title }</a></td>
					</c:when>
					<c:when test="${draft.isSign3 ne null }">
						<td class="title"><a href="done/view.do?no=${draft.no}" >${draft.title }</a></td>
					</c:when>
				</c:choose>
